...
only — the client still just sends the hostname (`socks5h` / HTTP proxy). Keep
the suffix on the tunnel set (`routed_domains`), since the whitelist is enforced
on the requested hostname before resolution. See `server.toml.example`.
