...
| `proxy/signaling.rs` | ALPN builder, length-prefixed `Hello`/`HelloResponse`, per-stream `Target` codec, `REP_*` codes |
...
### 1. ALPN "port knock"
The ALPN value is `flextunnel/1/<alpn_token>` (`signaling::build_alpn`). A peer
that doesn't share the token fails during the QUIC/TLS handshake, **before any
stream opens** — a cheap pre-auth filter. The token is 14 Base64URL chars
(8 random bytes + CRC16); see `auth.rs`.
...
- **Two shared secrets:** the ALPN token (pre-handshake knock) and a per-client
  auth token (checked in the handshake). Both are CRC16-checksummed Base64URL
  credentials generated by the CLI. All payload is encrypted by QUIC/TLS 1.3.
...
| auth token / ALPN token length | 47 / 14 chars | `auth.rs` |

## Relation to ezvpn

flextunnel reuses ezvpn's iroh transport, ALPN-knock + auth-token scheme, and
